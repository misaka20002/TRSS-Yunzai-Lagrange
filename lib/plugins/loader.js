...
class PluginsLoader {
    ...
    if (e.message)
      for (const i of e.message) {
        switch (i.type) {
          ...
          case "reply":
            e.reply_id = i.id
            e.source = { // 加入 miao-yunzai 的 e.source 参数
            seq: i.id,
            time: i.id,
          }
      ...
      if (recallMsg > 0 && res?.message_id) {
        if (e.group?.recallMsg)
          setTimeout(() => {
            ...
            // if (e.message_id) e.group.recallMsg(e.message_id) // 注释掉撤回原消息的功能
          }, recallMsg * 1000)
        else if (e.friend?.recallMsg)
          setTimeout(() => {
            e.friend.recallMsg(res.message_id)
            // if (e.message_id) e.friend.recallMsg(e.message_id) // 注释掉撤回原消息的功能
          }, recallMsg * 1000)
      }
  ...
  async saveCount(e, type) {
    ...
    for (const i of [day, month, year, "total"]) {
      ...
      // 统计 Bot 在某群的发言
      if (type.startsWith("send") && e.group_id && e.self_id) key.push(`group:${e.group_id}:user:${e.self_id}:${i}`)
    }
